...
stock AddAmmoToWeapon(itemid, amount)
{
	...
	if(maxammo == 0)
	{
		dbg("weapon.core", 2, "[AddAmmoToWeapon] Adding %d ammo to item %d: ammoitem type: %d magsize: %d reserve: %d", amount, itemid, _:ammoitem, magsize, reserveammo);
	...
	else
	{
		...
		dbg("weapon.core", 2, "[AddAmmoToWeapon] Adding %d ammo to item %d: ammoitem type: %d reserve: %d max reserve: %d", amount, itemid, _:ammoitem, reserveammo, maxammo);
...
stock UpdatePlayerWeaponItem(playerid)
{
	...
	if(!IsValidItem(itemid))
	{
		dbg("weapon.core", 2, "[UpdatePlayerWeaponItem] ERROR: Invalid item ID %d", itemid);
		return 0;
	}
...
{
	dbg("global", CORE, "[OnPlayerUpdate] in /weapon.core");
...
_UpdateWeaponUI(playerid)
{
	...
	dbg("weapon.core", 1, "[_UpdateWeaponUI] updating weapon UI for item %d", itemid);
	...
	dbg("weapon.core", 2, "[_UpdateWeaponUI] item %d magammo %d reserve %d", itemid, GetItemWeaponItemMagAmmo(itemid), GetItemWeaponItemReserve(itemid));
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	...
	if(oldkeys & KEY_NO)
	{
		dbg("weapon.core", 1, "[OnPlayerKeyStateChange] dropping item %d magammo %d reserve %d", itmw_DropItemID[playerid], GetItemWeaponItemMagAmmo(itmw_DropItemID[playerid]), GetItemWeaponItemReserve(itmw_DropItemID[playerid]));
		if(IsValidItem(itmw_DropItemID[playerid]))
		{
			dbg("weapon.core", 2, "[OnPlayerKeyStateChange] dropping item %d magammo %d reserve %d", itmw_DropItemID[playerid], GetItemWeaponItemMagAmmo(itmw_DropItemID[playerid]), GetItemWeaponItemReserve(itmw_DropItemID[playerid]));
...
_unload_DropHandler(playerid, itemid)
{
	...
	dbg("weapon.core", 1, "[OnPlayerDropItem] dropping item %d magammo %d reserve %d", itemid, GetItemWeaponItemMagAmmo(itemid), GetItemWeaponItemReserve(itemid));
...
{
	dbg("weapon.core", 1, "[_UnloadWeapon] unloading item %d magammo %d reserve %d", itemid, GetItemWeaponItemMagAmmo(itemid), GetItemWeaponItemReserve(itemid));
...
hook OnItemNameRender(itemid, ItemType:itemtype)
{
	...
	dbg("weapon.core", CORE, "rendering name for weapon item %d type %d itemweaponid %d", itemid, _:itemtype, itemweaponid);
...
{
	dbg("weapon.core", 3, "GetItemWeaponItemMagAmmo itemid:%d", itemid);
...
{
	dbg("weapon.core", 3, "SetItemWeaponItemMagAmmo itemid:%d, amount:%d", itemid, amount);
...
{
	dbg("weapon.core", 3, "GetItemWeaponItemReserve itemid:%d", itemid);
...
{
	dbg("weapon.core", 3, "SetItemWeaponItemReserve itemid:%d, amount:%d", itemid, amount);
...
{
	dbg("weapon.core", 3, "GetItemWeaponItemAmmoItem itemid:%d", itemid);
...
{
	dbg("weapon.core", 3, "SetItemWeaponItemAmmoItem itemid:%d, itemtype:%d", itemid, _:itemtype);
	SetItemArrayDataSize(itemid, 4);
